...
**Trained on.** 23,200 synthetic pairs over nineteen forms, labels known by construction, made from templates by `scripts/synth.py` (seed 7): honest answers given bare or after the question's stem, and evasive, deferred, general, empty and off-question answers labelled *no answer*. Three rows in ten reach the reader without their stem, honest answers and non-answers alike. No person's words, and no animal product named. Eighteen forms describe everyday objects, moments and events; one is the record of an artwork. Two epochs, batch 32, learning rate 2e-5, maximum length 384, on CPU, 833 seconds on 18 ARM cores. See `DATA-CARD.md`.

**Measured** on three test sets it never saw, at one threshold, **12.67**, the smallest margin that lets no non-answer through in any of them. On **test set v1** (360 rows over the first ten everyday forms) it counts **155 of 180** honest answers. On the **artwork set** (48 rows) it counts **21 of 24**. On the **unseen set** (168 rows over four forms it was never trained on) it counts **59 of 96**: 32 of 48 read with the form's stem and 27 of 48 without. It counts no non-answer in any set. The previous reader, 0.3.4, counted 138, 20 and 39 on the same sets at 16.24, the threshold that held it at zero on all three. A threshold belongs to the reader it was measured for, so 12.67 and 16.24 are not comparable as numbers. The untrained deepset reader, given the same stems, counts 16 of 180 on test set v1 at 10.60, its own zero-false-present threshold. No phrase in any test set appears in the training data, and no training question repeats a question from an unseen form.

**Scope.** The reader is for forms: a short list of questions written in advance, each asking for a concrete particular (a name, a number, a place, a time, a colour, a comparison with a named thing), answered in short typed phrases. It is not a general completeness checker and does not judge whether an answer is right or enough. It was trained on nineteen forms: eighteen about everyday objects, moments and events, and the record of an artwork. On forms it was not trained on it refuses non-answers as before but misses more real ones, 59 of 96 on the unseen set against 155 of 180 on test set v1. See the README's *Scope* section.
...
- The artwork set has 24 real answers and the unseen set 96. The artwork change, 20 to 21 of 24, is one answer and says nothing on its own.
...
counted = span is not None and margin > 12.67
```
