...
through anywhere — and each set is then reported at that threshold. usage:
    python scripts/eval_reader.py tests/testset-v1.jsonl [more.jsonl ...] [model] [--no-stem] [--show-misses] [--json results.json]"""
...
model = next((a for a in sys.argv[1:] if not a.startswith("--") and not a.endswith(".jsonl") and a != json_out), BASE)
